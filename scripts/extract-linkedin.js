...
const OUT_JSON = path.join(DATA_DIR, 'experience.json');

if (!fs.existsSync(PDF_PATH)) {
  console.error('Profile.pdf not found in data/');
  process.exit(1);
}

function round(n, prec = 2) {
...
for (let p = 1; p <= doc.numPages; p++) {
  ...
  content.items.forEach((it) => {
    const y = round(it.transform[5], 1); // group by y position
...
for (let idx = 0; idx < lines.length; idx++) {
  ...
  let tIdx = idx - 1;
  while (tIdx >= 0 && (isNoise(lines[tIdx].text) || lines[tIdx].fontSize <= dateFont + 0.1)) {
    tIdx--;
  }
  if (tIdx < 0) continue;
  const firstLine = lines[tIdx];
  const titleParts = [firstLine.text];
  ...
  while (tIdx >= 0) {
    ...
    if (Math.abs(fSize - titleFont) > 0.2) break; // different font -> stop (company or other)
...
fs.writeFileSync(OUT_JSON, JSON.stringify({ positions }, null, 2));
console.log(`Wrote ${positions.length} positions to experience.json`); 